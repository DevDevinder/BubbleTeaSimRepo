     ...
     void PourMilk()
    {
        ...
        {// raycast from location(center screen)
            ...
            {//check if it hit a colider containing the hotwatercheck script(kettle machine)
                HotWaterCheck phit = hit.collider.GetComponent<HotWaterCheck>();
                //if hit
                if (phit != null)
                {
